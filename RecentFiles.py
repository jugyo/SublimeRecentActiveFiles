...
class RecentFilesEventListener(sublime_plugin.EventListener):
    def on_activated(self, view):
        if view.file_name():
            view.window().run_command("recent_files", { "file_name": view.file_name() })

class RecentFilesCommand(sublime_plugin.WindowCommand):
    ...
    def run(self, file_name=None):
        if file_name:
            ...
            if self.window.active_view() is not None:
                ...
                files = filter(lambda f: f != active_file, self.recent_files)
            else:
